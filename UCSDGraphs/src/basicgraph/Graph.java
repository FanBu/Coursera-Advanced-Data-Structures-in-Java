...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for(int i = 0; i < numVertices; i++){
			int count = 0;
			count += getNeighbors(i).size();
			count += getInNeighbors(i).size();
			degSeq.add(count);
		}
		quickSort(degSeq);
		return degSeq;
	...
	int partition(List<Integer> L, int low, int high) {
		int pivotKey = L.get(low);
		while (low < high) {
			while (low < high && L.get(high) <= pivotKey) high--;
			L.set(low, L.get(high));
			while (low < high && L.get(low) >= pivotKey) low++;
	...
	public void qSort(List<Integer> L, int low, int high){
		if (low < high){
